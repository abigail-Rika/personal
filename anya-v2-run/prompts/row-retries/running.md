Create Codex pet row `running` for `anya`: exactly 6 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the existing Anya identity exactly: pink-haired pixel humanoid companion, same face, palette, proportions, outfit, and style. Upgrade only by adding natural gaze directions.. Preserve silhouette, face, palette, material, proportions, markings, and props.
