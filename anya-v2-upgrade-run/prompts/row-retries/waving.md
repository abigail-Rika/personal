Create Codex pet row `waving` for `anya`: exactly 4 full-body frames in one horizontal strip on flat pure cyan #00FFFF.

Use the attached canonical base for identity and the layout guide only for spacing. Same pet in every frame: Preserve the existing Anya identity exactly: pink-haired pixel girl with black cat-ear bows, green eyes, black-and-gold outfit, white shoes, and the small round green companion held at her front. Add subtle, anatomically grounded gaze directions while keeping her lower body and prop attachment stable.. Preserve silhouette, face, palette, material, proportions, markings, and props.
